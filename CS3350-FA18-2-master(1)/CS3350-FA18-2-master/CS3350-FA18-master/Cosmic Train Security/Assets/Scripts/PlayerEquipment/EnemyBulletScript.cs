...
public class EnemyBulletScript : MonoBehaviour
{
    ...
    Rigidbody2D rb2d;
    //particle effect for when the bullet hits the wall
    public ParticleSystem enemySplashEffect;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            ParticleSystem splashEffectParticleSystem = Instantiate(enemySplashEffect, transform.position, transform.rotation);
        }
        // Destory on impact with Wall.
        if (collision.gameObject.tag == "Wall" || collision.gameObject.tag == "Locker" ||
            collision.gameObject.tag == "Player" || collision.gameObject.tag == "Charge Station")
        {
            Destroy(gameObject);
        }
